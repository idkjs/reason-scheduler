// https://github.com/KinoAR/re-verv
module EBase = {
  type vervContainerT('a) =
    | Container('a);
...
module EventEmitter = (E: {type t;}) => {
  module PEEmitter =
    EventEmitterApi.Events.EventEmitter({
      type nonrec t = E.t;
    });

  let emit0 = eventName => flatMap(PEEmitter.emit0(_, eventName));
  let emit1 = (eventName, arg1) =>
    flatMap(PEEmitter.emit1(_, eventName, arg1));
  let emit2 = (eventName, arg1, arg2) =>
    flatMap(PEEmitter.emit2(_, eventName, arg1, arg2));
  ...
  let addEventListener = (eventName, fn) =>
    map(PEEmitter.addEventListener(_, eventName, fn));
  ...
  let removeAllListenersStr = eventName =>
    map(PEEmitter.removeAllListenersStr(_, eventName));
  let removeListenerStr = eventName =>
    map(PEEmitter.removeListenerStr(_, eventName));
};
